...
import { getAnimeInfo, getEpisodes } from '../../Components/Func/AnimeVideoFunc';
import { NAVIGATION } from '../../Constants';

export function AnimeDetails({ navigation, route }) {
    ...
    const getEpisodesForPage = async (page, index) => {
        ...
        try {
            let episodes = await getEpisodes({ ep_start: page.epStart, ep_end: page.epEnd, id: data.id, default_ep: 0, alias: data.alias, dispatch })
            if (!episodes) return
            setData({
                ...data, episodes,
        ...
        <SafeAreaView style={{ flex: 1, backgroundColor: '#222' }} edges={['top']}>
            <View style={{ flex: 1 }}>
                ...
                <ScrollView
                    ...
                    {TabSelected !== 0 ? null :
                        !data?.plotSummary ? null : < Text style={styles.text}>
                    ...
                    {TabSelected !== 1 ? null : (
                        <View>
                            ...
                            <View style={{ flexDirection: "row", flexWrap: "wrap", gap: 12, marginTop: 12 }}>
                                ...
                                {episodeLoading ?
                                    <View style={{ flexDirection: "row", gap: 12, justifyContent: "center", alignItems: "center", flex: 1 }}>
                                    ...
                                    data?.episodes?.map((episode, index) => (
                                        ...
                                        >
                                            <View style={[{ gap: 12, flexDirection: "row", justifyContent: "space-between", alignItems: "center" }]}>
                                                <Text style={{ color: '#FFF', fontSize: 14, fontWeight: 'bold' }}>{episode.episodeNumber}</Text>
                                                <Text style={{ color: 'silver', fontSize: 12, fontWeight: 'bold' }}>{episode.episodeCategory}</Text>
                                            </View>
                                        </TouchableOpacity>
                                    ))}
                            </View>
